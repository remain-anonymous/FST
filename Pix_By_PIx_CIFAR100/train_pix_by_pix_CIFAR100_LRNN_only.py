...
configs = [  
            ...
            {'hid_feature_size': 128, 'nblocks': 4},
...
for config in configs:
    ...
    model = Block_FSU( features, seq_len, out_dim, hid_feature_size, nblocks, dropout_rate=.3).to(DEVICE)
    ...
    while True:
        for src, tgt in train_loader:
            ...
            if updates%100==0: #if training_time >= eval_every*nevals:
                ...
                nevals+=1
                pickle.dump(results, open(f'results/CIFAR100_noise_{noise}_LRNN_only_nblocks_{nblocks}_hid_feature_size_{hid_feature_size}_results.pkl', 'wb'))
